...
export default function Dashboard() {
    ...
    if (loadingSession) return <p>Cargando dashboard...</p>;
    ...
    if (user.role === "admin") {
        ...
        const sortedAdminUsers = [...filteredAdminUsers].sort((a, b) => {
            ...
            if (order === "workoutsAsc") return countUserWorkouts(a.id) - countUserWorkouts(b.id);
            if (order === "workoutsDesc") return countUserWorkouts(b.id) - countUserWorkouts(a.id);
            return 0;
        });
        ...
        return (
            <>
                ...
                <AdminDashboard users={sortedAdminUsers} countUserWorkouts={countUserWorkouts} />
            </>
        );
    }
    ...
    if (user.role === "trainer") {
        ...
        const totalExercises = exercises.length;
        const trainerBeginnerWorkouts = workouts.filter((w) => w.level === "Principiante").length;
        const trainerIntermediateWorkouts = workouts.filter((w) => w.level === "Intermedio").length;
        const trainerExpertWorkouts = workouts.filter((w) => w.level === "Experto").length;
        ...
        const filteredTrainerWorkouts = searchedTrainerWorkouts.filter((workout) =>
            levelFilter === "all" ? true : workout.level === levelFilter
        );
        ...
        const sortedTrainerWorkouts = [...filteredTrainerWorkouts].sort((a, b) => {
            ...
            if (order === "durationAsc") return Number(a.duration) - Number(b.duration);
            if (order === "durationDesc") return Number(b.duration) - Number(a.duration);
            return 0;
        });
    ...
    const userBeginnerWorkouts = ownWorkouts.filter((w) => w.level === "Principiante").length;
    const userIntermediateWorkouts = ownWorkouts.filter((w) => w.level === "Intermedio").length;
    const userExpertWorkouts = ownWorkouts.filter((w) => w.level === "Experto").length;
